    ...
    private boolean probeWriteCharacteristic(UUID serviceUuid, UUID characteristicUuid, byte[] frame, String channel) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 33) {
                ...
                int result = gatt.writeCharacteristic(writeCharacteristic, frame, writeType);
                writeLog("PROBE_WRITE_RESULT\tresult=" + result + "\twrite_type=" + writeType);
            } else {
                writeCharacteristic.setWriteType(noResponse ? BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE
                        : BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
                writeCharacteristic.setValue(frame);
                boolean started = gatt.writeCharacteristic(writeCharacteristic);
                writeLog("PROBE_WRITE_STARTED\tstarted=" + started + "\twrite_type=" + writeCharacteristic.getWriteType());
            }
        } catch (SecurityException e) {
